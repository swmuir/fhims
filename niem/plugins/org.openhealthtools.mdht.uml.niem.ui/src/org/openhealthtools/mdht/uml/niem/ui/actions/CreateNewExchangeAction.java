...
public class CreateNewExchangeAction implements IObjectActionDelegate {
	
	protected IWorkbenchPart activePart;
	protected ISelection currentSelection;
	protected Package selectedPackage;
	
	public CreateNewExchangeAction() {
		super();
	}
	...
	public void run(IAction action) {
		try {
			...
			IUndoableOperation operation = new AbstractEMFOperation(editingDomain, "Create New NIEM Exchange") {
			    protected IStatus doExecute(IProgressMonitor monitor, IAdaptable info) {

					// prompt for new class name
					String className = null;
					InputDialog inputDialog = new InputDialog(activePart.getSite().getShell(), 
							"New NIEM Exchange", "Enter exchange name", "", null);
